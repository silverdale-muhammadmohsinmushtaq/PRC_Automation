...
test.describe.serial('Odoo End-to-End QA', () => {
  test.beforeAll(async ({ browser }) => {
	...
	test('Verify the user can search the LPN in Repair Shop', async () => {
		...
		await handleRepairShopWorkcenterPopup(page, { timeout: 15000, checkAll: true });
...
test('Verify the user can Generate and download the "Repair Report History"', async () => {
	...
	await page.getByRole('menuitem', { name: 'Repair Report History' }).click();
	...
	await expect(page.locator('text=Vendor_October_2025_Repair_Cost_Template.xlsx')).toBeVisible();
	...
	const downloadLink = page.getByRole('link', { name: ' Vendor_October_2025_Repair_Cost_Template.xlsx' });
